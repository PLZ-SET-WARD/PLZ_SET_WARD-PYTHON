...
api_key = 'RGAPI-0ee0d6ae-2087-48e7-8a93-9c81ad471032'
request_header = {
...
def summoner_v4_by_summoner_name(summonerName):
    ...
    url = f"https://kr.api.riotgames.com/lol/summoner/v4/summoners/by-name/%7BencodingSummonerName%7D"
...
def matchgame():
    ...
    url = f"https://asia.api.riotgames.com/lol/match/v5/matches/by-puuid/%7Bpuuid%7D/ids?count=50&api_key=%7Bapi_key%7D"
    return requests.get(url, headers=request_header).json()
...
def checkgametype(matchid):
    encodingPuuid = parse.quote(puuid)
    url = f"https://asia.api.riotgames.com/lol/match/v5/matches/%7Bmatchid%7D?api_key=%7Bapi_key%7D"
    return requests.get(url, headers=request_header).json()
